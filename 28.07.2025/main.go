...
type kullanıcıYarat struct {
	Kullanıcıadı string `json:"isim" validate:"required,min=2,max=32,alphanum"`
	Soyisim      string `json:"soyisim" validate:"required,min=2,max=32"`
	Email        string `json:"email" validate:"required,email"`
	Sifre        string `json:"sifre" validate:"required,min=8,max=16"`
...
func main() {
	...
	app.Post("/user", func(c *fiber.Ctx) error {
		...
		if len(validationErrors) > 0 && validationErrors[0].Hata {
			var detaylar []HataYanıtıDetayları
			for _, e := range validationErrors {
				detay := HataYanıtıDetayları{
					Description: fmt.Sprintf("Alan '%s' için '%s' kuralı geçersiz. Değer: %v", e.Alan, e.Etiket, e.Value),
					AlanHatası:  e.Alan,
				}
				detaylar = append(detaylar, detay)
			}

			hataCevabı := HataYanıtı{
				Status:        fiber.StatusBadRequest,
				HataDetayları: detaylar,
			}
			return c.Status(fiber.StatusBadRequest).JSON(hataCevabı)
		}
